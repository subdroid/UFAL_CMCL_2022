...
class model(nn.Module):
    def __init__(self,base):
        ...
        self.Linear = nn.Sequential(nn.Linear(115200, 768),nn.ReLU())
...
if __name__ == '__main__':
    ...
    train,dev = load_csv("FFDStd")
    train_tokens= B_tok.batch_encode_plus(train['word'].tolist(),max_length=150,padding='max_length')['input_ids']
    train_dataset = loader_module(train_tokens, train['FFDStd'])
    train_loader = DataLoader(train_dataset, batch_size=16, shuffle=True)
    dev_tokens= B_tok.batch_encode_plus(dev['word'].tolist(),max_length=150,padding='max_length')['input_ids']
    dev_dataset = loader_module(dev_tokens, dev['FFDStd'])
    dev_loader = DataLoader(dev_dataset, batch_size=16)
    
  
    max_epochs = 10

    SAVE_PATH = os.path.join(os.getcwd(),os.path.join("saved_models","FFDStd_10.pt"))
